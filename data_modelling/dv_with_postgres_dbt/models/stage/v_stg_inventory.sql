...
-- Debugging: log metadata to confirm structure
{{ log(metadata_dict, info=True) }}
{{ log("source_model: " ~ source_model, info=True) }}
{{ log("derived_columns: " ~ derived_columns, info=True) }}
{{ log("hashed_columns: " ~ hashed_columns, info=True) }}
